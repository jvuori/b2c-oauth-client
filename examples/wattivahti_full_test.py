...
def main() -> None:
    ...
    try:
        ...
        print(f"   Expires in: {(token.expires_at - datetime.now()).total_seconds():.0f} seconds")
        ...
        if token.refresh_token:
            ...
            print(f"   ⚠️  No refresh token in response")
        print()
    ...
    if client.is_token_valid(token):
        ...
        print(f"⚠️  Token is expired or will expire soon")
    print()
    ...
    try:
        ...
        if filtered_readings:
            ...
            for reading in filtered_readings[:5]:
                ...
                print(f"  {ts.strftime('%Y-%m-%d %H:%M')}: {reading['consumption_kwh']:.3f} {reading['unit']}")
